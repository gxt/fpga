...
#              -tclargs workspace/T010-sync rtl_out/core_mini_axi synth/rtl synth/xdc batch'
#
# 行为: ssh 202 → cd ~/fpga → mkdir workspace/<task> → nohup 后台执行 → 日志落
#       workspace/<task>/build.log → 立即返回（不等待）。
...
ssh -o BatchMode=yes gxt@192.168.200.202 \
    "mkdir -p ~/fpga/workspace/$TASK && cd ~/fpga/workspace/$TASK && { nohup bash -c '$CMD' > build.log 2>&1 < /dev/null & } && echo \$! > pid && echo '==> 已启动 $TASK (PID '\$!')'"
