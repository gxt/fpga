...
#   （默认 [b]uild_top.tcl.*work/<task>，[b] 正则字符类排除 ssh 自身）
#
...
ssh -o BatchMode=yes gxt@192.168.200.202 \
    "PIDFILE=~/fpga/work/$TASK/pid; while { [ -f \$PIDFILE ] && kill -0 \$(cat \$PIDFILE) 2>/dev/null; } || { [ ! -f \$PIDFILE ] && pgrep -f '$PAT' >/dev/null 2>&1; }; do sleep 15; done; echo '==> $TASK 结束'; grep -E '^ERROR|DONE|build_exit' ~/fpga/work/$TASK/build.log | tail -8"
