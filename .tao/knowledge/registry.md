...
机器202任务路由表，dispatch/complete 据此同步/拉取任务文件。无机器202任务时留空即可：

| 执行环境 | 传输命令（push 任务文件到机器202） | 机器202执行方 |
| --- | --- | --- |
| `机器202 Vivado · zzx-NF5280 (202)` | **git 局域网同步为主**（202 从 201 `git pull`；**fpga 主仓库仅限 201 局域网 push/pull**，coralnpu submodule 及其他软件走外网）；`synth/sync.sh`（rsync）辅助推产物 | 机器201 ssh 直连执行（路径 A，无机器202 opencode；拓扑见 `synth-server.md`） |

- 传输命令**以本表为准**（git 局域网同步 / scp / rsync），不要假设一律 rsync
- pull（机器202拉回机器201）由 push 命令反推：`scp [-P port] user@host:remote_path/<file> <local_file>`
- 主机地址只写在本表，不写进命令/角色文件

## 机器职责分工（2026-08-20 调整）

### 201（192.168.200.201 · fpga201，机器201）
...
## 机器202资源（zzx-NF5280 · 192.168.200.202）

- 连接：`ssh gxt@192.168.200.202`（密钥认证，免密）
- Vivado：v2025.1（Build 6140274），`/tools/Xilinx/2025.1/Vivado/bin/vivado`，与机器201一致
...
- 机器202工作根：`~/fpga/`（git 仓库）；任务子目录与 Vivado 工程规范见 `synth-server.md` §202 工作规范
