...
- **跨仓库通用规则**（所有项目共同遵守）已移至全局 `~/.config/opencode/instructions/`（源：`~/t.a.o/opencode/instructions/`）：
  - `directories.md`：目录使用约定（禁 `~/` 临时目录）+ 目录/产物命名规范
  - `workflow.md`：任务文件→实现→reviewer 验收→返工闭合 + 提醒机制
  - `remote-tasks.md`：远程任务 nohup 启动/PID 判定/非阻塞查询/时间协作
  - `debugging.md`：交互调试实时输出、禁 tail 截断
- **本文件保留项目特有执行细节**（202 服务器时间表、run202 工具、S2C 上板流程、目录结构实例化）。
