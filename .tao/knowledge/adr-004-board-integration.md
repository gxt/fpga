...
- 状态：已接受
- 日期：2026-08-16（2026-08-16 修订：去 Zynq PS 选项；明确 T010 直接产出上板 bitstream）
- 相关任务：T010、T012、T013
...
3. **T010 直接产出上板 bitstream**：器件适配面向 `core_mini_axi` + AXI 桥接顶层 + S2C 引脚/时钟适配（目标器件 `xc7v2000tflg1925-1`），产出可直接上板的 bitstream；T009 官方器件（xcvu13p）综合结果仅作资源基线对比，不用于上板。
4. AXI 桥接的 host 侧按板卡能力收窄为：
   - 板卡上现成的软核（MicroBlaze），或
   - 外部 JTAG/驱动脚本通过调试口直接读写，或
   - 简单的状态机主控（视板卡能力选择，任务内落实）。
   （板卡为纯 Virtex-7 FPGA，无 Zynq PS 硬核，故排除。）
5. 上板结果须与 RTL 仿真（T003/T006/T007 同程序）对齐，作为正确性证据。
