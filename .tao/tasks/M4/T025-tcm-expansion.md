...
- 建议 **8K/1M**：ITCM 8K 够（代码 1-6K）+ DTCM 1M（必需）→ BRAM ~265 RAMB36（21%），比 1M/1M 省一半
...
| # | 修改 | 内容 |
| --- | --- | --- |
| 1 | fork SoCChiselConfig | `dtcmSizeKBytes=1024`（itcm 保持 8K）→ 自动 highmem 布局 |
| 2 | 评测框架 bench_rvv.py | CSR 基址参数化：default 0x30008 ↔ highmem 0x200008（CTRL/STATUS/HALTED） |
...
- DTCM 1M 的 BRAM：~265 RAMB36（21%）——与 SRAM 256K（当前 74 RAMB36）叠加后 ~340（26%），余量充足
