...
本目录承载机器202（zzx-NF5280 · 192.168.200.202）的文件交换工作流。
机器202信息（地址、Vivado 版本、文件交换方式）登记在 `.tao/knowledge/registry.md`，
执行拓扑三要素见 `.tao/knowledge/synth-server.md`，本目录不重复登记主机地址。

## 文件

- `sync.sh` —— 同步工作流脚本（push/pull/exec），从 registry.md 自动解析机器202地址
...
```bash
cd synth
./sync.sh info                  # 确认机器202连通与 Vivado 版本
./sync.sh push all              # 推送 coralnpu 源码 + core_mini_axi RTL 产物
./sync.sh push rtl rvv_core_mini_axi   # 追加推送 RVV 变体 SV
./sync.sh exec "mkdir -p ~/fpga/work"  # 准备机器202工作目录
./sync.sh pull                  # 拉回机器202 work/ 全部结果到 out/
...
work/              # 机器202综合工作目录：工程、报告、bitstream（sync.sh pull）
```
