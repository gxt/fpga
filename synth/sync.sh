...
#
# 用法:
#   ./sync.sh info                    显示解析出的服务器信息
#   ./sync.sh push src                推送 coralnpu 源码到远端（rsync 增量）
#   ./sync.sh push rtl <name>...      推送 bazel RTL 产物到远端 rtl_out/
#                                     name: core_mini_axi | rvv_core_mini_axi
#   ./sync.sh push synth              推送 synth/ 脚本目录本身到远端
#   ./sync.sh push all                等价: push src + push rtl core_mini_axi
#   ./sync.sh pull [<subdir>]         从远端 work/ 拉回结果到本地 synth/out/
#   ./sync.sh exec "<远程命令>"        在远端执行命令（BatchMode 免密）
#
# 服务器信息从 ../.tao/knowledge/registry.md 自动解析（主机地址只登记在 registry），
# 不写入本脚本；可用环境变量覆盖:
#   SYNTH_SERVER=user@host      # 默认从 registry.md 解析
#   SYNTH_REMOTE_ROOT=~/fpga    # 远端工作根目录
#
...
# RTL 产物映射表（源 = 本地 bazel-out 固定路径；目标 = 远端 rtl_out/<key>/）
...
cmd_info() {
  [[ -n "$SERVER" ]] || die "无法从 $REGISTRY 解析服务器地址（可设 SYNTH_SERVER 覆盖）"
  say "服务器: $SERVER"
  say "远端工作根: $REMOTE_ROOT"
  say "远端 Vivado: $VIVADO"
...
cmd_push_rtl() {
  ...
  for name in "$@"; do
    ...
    for f in "${src_files[@]}"; do
      [[ -f "$RTL_OUT_SRC/$f" ]] \
        || die "本地产物缺失: $RTL_OUT_SRC/$f（先执行 bazel build //hdl/chisel/src/coralnpu:${name}_emit_verilog）"
    done
