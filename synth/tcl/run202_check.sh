...
#   判定方式: 优先 PID 文件（run202.sh 启动时写入 work/<task>/pid，kill -0 判存活）；
#             无 pid 文件时用 pid_pattern 兜底（默认 [b]uild_top.tcl.*work/<task>）
#
